    ...
    def test_fit_implicit_point_cloud(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            config = fit_implicit.TrainingConfig(
                ...
                checkpoint_options=fit_implicit.CustomCheckpointManagerOptions(
            ...
            for checkpoint_index in [0, 2, 4]:
                self.assertFileExists(
                    experiment_dir / f"checkpoints/{checkpoint_index}"
                )
                self.assertFileExists(
                    experiment_dir / f"meshes/mesh_{checkpoint_index}.ply"
                )

    def test_step(self):
        model = diffcd.networks.MLP(layer_size=10, n_layers=4, skip_layers=(2,))
        key = jrnd.PRNGKey(0)
        params = model.init(key, jnp.zeros(3) * 1.0)
        ...
        for i in range(model.n_layers):
            ...
            out_dim = (
                model.layer_size
                if i + 1 not in model.skip_layers
                else model.layer_size - 3
            )
            self.assertEqual(
                params["params"][f"dense_{i}"]["kernel"].shape, (in_dim, out_dim)
            )
            self.assertEqual(params["params"][f"dense_{i}"]["bias"].shape, (out_dim,))
        self.assertEqual(
            params["params"][f"dense_{model.n_layers}"]["kernel"].shape,
            (model.layer_size, 1),
        )
        self.assertEqual(
            params["params"][f"dense_{model.n_layers}"]["bias"].shape, (1,)
        )

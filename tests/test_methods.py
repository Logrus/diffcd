...
class TestMethods(unittest.TestCase):
    ...
    def test_igr_loss(self):
        f = lambda r, x: r - jnp.linalg.norm(x)
        x, r = jnp.array([1.0, 2.0, 3.0]), 2.0

        # l2
        igr_l2 = diffcd.methods.IGR(eikonal_weight=0.0, distance_metric="l2")
        igr_loss, _ = igr_l2.get_loss(f, r, x, x, x)
        self.assertAlmostEqual(
            igr_loss, jnp.linalg.norm(x - r * x / jnp.linalg.norm(x)), places=6
        )
        grad, _ = jax.grad(igr_l2.get_loss, argnums=1, has_aux=True)(f, r, x, x, x)
        self.assertAlmostEqual(grad, -1.0)

        # squared l2
        igr_sql2 = diffcd.methods.IGR(eikonal_weight=0.0, distance_metric="squared_l2")
        igr_loss, _ = igr_sql2.get_loss(f, r, x, x, x)
        self.assertAlmostEqual(
            igr_loss, jnp.linalg.norm(x - r * x / jnp.linalg.norm(x)) ** 2, places=6
        )
        ...
        f = lambda r, x: r**2 - diffcd.closest_point.sq_norm(x)
        x = jnp.array([0.0, 0.0, 0.0])
        (igr_loss, _), grad = jax.value_and_grad(
            igr_l2.get_loss, argnums=1, has_aux=True
        )(f, r, x, x, x)
    ...
    def test_pull_point_zero_grad(self):
        ...
        loss, grad = jax.value_and_grad(
            lambda x: ((diffcd.methods.pull_point(f, x) - jnp.ones(3)) ** 2).mean()
        )(x)
        np.testing.assert_almost_equal(loss, 1.0)
    ...
    def test_closest_point(self):
        ...
        closest_point, newton_state, valid = diffcd.closest_point.closest_point_newton(
            ...
            x,
            x,
            diffcd.closest_point.NewtonConfig(
                grad_norm_eps=eps, max_iters=max_iters, stop_when_converged=False
            ),
        )
        self.assertTrue(valid)
        np.testing.assert_array_almost_equal(
            closest_point, x / jnp.linalg.norm(x) * radius
        )
        self.assertTrue(newton_state.converged)
        self.assertEqual(newton_state.step, max_iters)
        laplacian_grad = jax.grad(diffcd.closest_point.laplacian, argnums=-1)(
            f, radius, x, newton_state.z_steps[-1]
        )
        ...
        closest_point, newton_state, valid = diffcd.closest_point.closest_point_newton(
            ...
            x,
            diffcd.closest_point.NewtonConfig(
                grad_norm_eps=eps, max_iters=max_iters, stop_when_converged=True
            ),
        )
        self.assertTrue(valid)
        np.testing.assert_array_almost_equal(
        ...
        laplacian_grad = jax.grad(diffcd.closest_point.laplacian, argnums=-1)(
            f, radius, x, newton_state.z_steps[newton_state.step]
        )
        self.assertLess(jnp.linalg.norm(laplacian_grad), eps)
    ...
    def test_closest_point_grad(self):
        ...
        radius = 2.0
        closest_point_grad = jax.jacrev(
            first(diffcd.closest_point.closest_point_newton), argnums=1
        )(f, radius, x, x, diffcd.closest_point.NewtonConfig())
        np.testing.assert_array_almost_equal(closest_point_grad, x / jnp.linalg.norm(x))

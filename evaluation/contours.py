...
def get_contour(inputs, sdf_values):
    ...
    for segment_points, segment_codes in zip(lines, codes):
        segment_edges = (
            np.vstack(
                [  # TODO: handle case when line is closed
...
def get_sample_points(contour: Contour, n_samples, seed=0):
    distances = np.linalg.norm(
        contour.vertices[contour.edges[..., 1]]
        - contour.vertices[contour.edges[..., 0]],
        axis=-1,
    )
    cumulative_distances = np.hstack([0.0, np.cumsum(distances)])

    sample_distances = (
        np.random.default_rng(seed).random(n_samples) * cumulative_distances[-1]
    )
    edge_indices = np.searchsorted(cumulative_distances, sample_distances) - 1

    alphas = (
        (sample_distances - cumulative_distances[edge_indices])
        / distances[edge_indices]
    )[..., None]

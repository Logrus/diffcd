...
class Meshing:
    ...
    lower_bound: Tuple[float, float, float] = (-0.8, -0.8, -0.8)
    upper_bound: Tuple[float, float, float] = (0.8, 0.8, 0.8)
...
def iter_grid(lower, upper, n, batch_size):
    ...
    for i in range(0, n, axis_batch_size):
        grid_values = jnp.meshgrid(
            axis_values[0][i : i + axis_batch_size], *axis_values[1:], indexing="ij"
        )

...
def normal_angle_deg(normals1, normals2):
    dot_products = np.einsum(
        "...i, ...i -> ...", normalize(normals1), normalize(normals2)
    )
...
def compute_shape_metrics(
    dist_fn,
    metric_name,
    shape1: trimesh.Trimesh,
    shape2: trimesh.Trimesh,
    num_mesh_samples,
):
    """
    ...
    if (
        (shape1 is None)
        or (shape2 is None)
        or (len(shape1.vertices) == 0)
        or (len(shape2.vertices) == 0)
    ):
        distance_metric, distance_square_metric, normal_angle_metric = (
            np.nan,
            np.nan,
            np.nan,
        )
        ...
        distance_square_metric = dist_fn(
            [dist_fn(distances12**2), dist_fn(distances21**2)]
        )
        ...
        if (face_indices1 is not None) and (face_indices2 is not None):
            ...
            normal_angle_metric = dist_fn(
                [dist_fn(normal_angle12), dist_fn(normal_angle21)]
            )

            # compute normal metric again with the normals of one of the meshes flipped, and select whichever is smallest
            normal_angle12 = normal_angle_deg(normals1, -normals2[closest_indices2])
            normal_angle21 = normal_angle_deg(-normals2, normals1[closest_indices1])
            normal_angle_metric_flipped = dist_fn(
                [dist_fn(normal_angle12), dist_fn(normal_angle21)]
            )

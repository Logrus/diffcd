...
def add_text(text, location, color=None, name=""):
    ...
    color_mat.diffuse_color = (0.0, 0, 0, 0) if color is None else color
...
def add_mesh(
    mesh: trimesh.Trimesh, color, name="mesh", center=False, offset=None, scale=1.0
):
    if offset is None:
...
def plot_mesh(
    ...
    if with_text:
        font_obj = add_text(
            text, offset + np.array([-0.0, -0.8, -0.62]), name=name, color=text_color
        )
    if train_points:
...
def setup_scene(
    clear=False, camera_location=None, camera_rotation=None, resolution=None
):
    ...
    bpy.data.objects["Sun"].data.energy = 30.0

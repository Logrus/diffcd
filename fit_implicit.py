...
class TrainingConfig:
    ...
    gt_mesh: datasets.EvaluationMesh = field(
        default_factory=lambda: datasets.EvaluationMesh()
    )
...
def load_config(experiment_dir: Path, name: str = "config"):
    try:
        ...
        print(
            f'WARNING: failed to load config from yaml config from {experiment_dir} due to "{e}", probably a result of version mismatch. Loading pickle file instead.'
        )
...
def get_gpu_memory():
    ...
    memory_free_values = {
        f"gpu{i}": int(x.split()[0]) for i, x in enumerate(memory_free_info)
    }
...
def eval(
    gt_mesh: Trimesh,
    n_samples: int,
    meshing_config: meshing.Meshing,
    f: Callable,
    transform: Callable,
    batch_index: int,
):
...
def save_checkpoint(
    checkpoint_manager, train_state, checkpoint_info, save_args, batch_index
):
...
def cos_with_warmup(init_lr, warm_up, max_iters, step):
    lr = jnp.where(
        step < warm_up,
        step / warm_up,
        0.5 * (jnp.cos((step - warm_up) / (max_iters - warm_up) * jnp.pi) + 1),
    )
    return lr * init_lr
...
def run(config: TrainingConfig):
    ...
    timestamp_str = (
        datetime.now().strftime("%Y-%m-%d_%H:%M:%S") if config.with_timestamp else ""
    )
    ...
    save_args = orbax_utils.save_args_from_target(
        {"model": train_state, **checkpoint_info}
    )
    ...
    (key,) = jrnd.split(key, 1)
    method_state = config.method.init_state(
        key, train_points, config.experiment_dir if config.copy_datasets else None
    )
    ...
    for batch_index in tqdm(range(config.n_batches)):
        if checkpoint_manager.should_save(batch_index):
            ...
            metrics, estimated_mesh = eval(
                config.gt_mesh.mesh,
                config.gt_mesh.n_samples,
                config.eval_meshing,
                partial(apply_fn, train_state.params),
                config.dataset.undo_normalization,
                batch_index,
            )
            if config.save_ply:
                meshing.save_ply(
                    estimated_mesh,
                    config.experiment_dir / f"meshes/mesh_{batch_index}.ply",
                )
            if config.wandb_project is not None:
        ...
        (key,) = jrnd.split(key, 1)
        method_state, *batch = config.method.get_batch(
            train_state, method_state, key, config.batch_size
        )
        ...
        if nan_grads:
            ...
            raise ValueError(
                "nan encountered in gradients. Checkpoint saved for debugging."
            )
        batch_metrics = {
            "step": batch_index,
            **batch_metrics,
    ...
    if config.save_ply:
        meshing.save_ply(
            estimated_mesh, config.experiment_dir / f"meshes/mesh_{batch_index}.ply"
        )
    if config.wandb_project is not None:
        wandb.log({"eval": metrics})
    ...
    meshing.save_ply(
        final_mesh, config.experiment_dir / f"mesh_final_{batch_index}.ply"
    )
    pd.DataFrame([final_metrics]).to_csv(
        config.experiment_dir / f"eval_metrics_final_{batch_index}.csv"
    )

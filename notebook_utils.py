...
def plot_sdf_surface(
    ...
    inputs,
    sdf_values,
    n_positive_levels=10,
    n_negative_levels=10,
    surface_width=2,
    with_0_level=True,
    hide_ticks=True,
    **kwargs,
    ...
    contour = ax.contourf(
        inputs[..., 0],
        inputs[..., 1],
        NonSymmetricNormalize()(sdf_values),
        cmap="RdBu_r",
        levels=np.hstack(
            [
                np.linspace(0, 0.5, n_negative_levels),
                np.linspace(0.5, 1.0, n_positive_levels + 1)[1:],
            ]
        ),
    )
...
def get_basis(mu0, mu1, mu2, preserve_angle=True, preserve_norm=True):
    ...
    x = np.linalg.lstsq(
        np.hstack([u1[:, None], u2[:, None]]),
        np.hstack([(mu1 - mu0)[:, None], (mu2 - mu0)[:, None]]),
    )[0]
    x1, x2 = x[:, 0], x[:, 1]
...
def tabilize(results, precisions, rank_order, suffixes=None, hlines=[]):
    ...
    data_quant = np.round((data * 10.0 ** (np.array(precisions)[None, :]))) / 10.0 ** (
        np.array(precisions)[None, :]
    )
    if suffixes is None:

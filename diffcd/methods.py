...
def any_nans(pytree):
    ...
    return jnp.array(
        jax.tree_util.tree_flatten(jax.tree_map(lambda a: jnp.isnan(a).any(), pytree))[
            0
        ]
    ).any()
...
def safe_apply_grads(state, grads):
    ...
    state = jax.lax.cond(
        nan_grads, lambda: state, lambda: state.apply_gradients(grads=grads)
    )
...
class DiffCD:
    ...
    surface_sampling: SurfaceSamplingConfig = field(
        default_factory=lambda: SurfaceSamplingConfig()
    )

    def init_state(self, key, point_cloud, save_dir=None):
        local_sigma = (
            samplers.compute_local_sigma(point_cloud, self.sampling.k)
            * self.sampling.local_sigma_scale
        )
    ...
    def get_surface_samples(
        self, key, apply_fn, params, mesh_samples, point_cloud, batch_size
    ):
        ...
        closest_train_points = vmap(point_cloud_closest_point, in_axes=(0, None))(
            surface_samples, point_cloud
        )
        return SurfaceSamples(surface_samples, closest_train_points, valid)

    def get_batch(
        self, train_state: ShapeTrainState, state: DiffCDState, key, batch_size
    ):
        ...
        if self.s2p_weight != 0:
            if train_state.step % self.surface_sampling.mesh_interval == 0:
                ...
                mesh_samples = trimesh.sample.sample_surface(
                    sampling_mesh, self.surface_sampling.num_samples
                )[0]
                state = DiffCDState(
                    state.point_cloud,
                    state.local_sigma,
                    state.point_cloud_kd_tree,
                    mesh_samples,
                )
    ...
    def closest_point_loss(self, f, params, query_point):
        closest_point, newton_state, valid = closest_point_newton(
            f, params, query_point, None, self.newton_config
        )
    ...
    def surface_sample_loss(self, f, params, surface_sample_point, point_cloud_point):
        ...
        loss = distance_loss(
            surface_sample_point, point_cloud_point, self.distance_metric
        )
        return loss
    ...
    def batch_loss(
        ...
        if self.p2s_loss == "closest-point":
            p2s_losses, newton_state, valid = jax.vmap(
                self.closest_point_loss, in_axes=(None, None, 0)
            )(apply_fn, params, query_points)
            ...
            p2s_loss = implicit_distance(
                partial(apply_fn, params), query_points, self.distance_metric
            )
        else:
            raise ValueError(f"Unrecognized p2s metric {self.p2s_metric=}.")

        if self.s2p_weight != 0.0:
            s2p_losses = vmap(self.surface_sample_loss, in_axes=(None, None, 0, 0))(
                apply_fn,
                params,
                surface_samples.points,
                surface_samples.closest_train_points,
            )
        ...
        if self.surface_area_weight != 0.0:
            surface_area_loss = get_implicit_surface_area_loss(
                partial(apply_fn, params), uniform_samples, self.alpha
            ).mean()
        else:
            surface_area_loss = 0.0

        loss = p2s_loss
        loss += self.s2p_weight * s2p_loss
        loss /= 1.0 + self.s2p_weight
    ...
    def step(
        self,
        state: ShapeTrainState,
        query_points,
        sample_points,
        uniform_samples,
        surface_points,
    ):
        grads, metrics = jax.grad(self.batch_loss, argnums=1, has_aux=True)(
            state.apply_fn,
            state.params,
            query_points,
            sample_points,
...
class IGR:
    ...
    def get_loss(
        self, apply_fn, params, query_points, sample_points, uniform_sample_points
    ):
        implicit_distance_loss = implicit_distance(
            partial(apply_fn, params), query_points, self.distance_metric
        )
        eikonal_loss = vmap(get_eikonal_loss, in_axes=(None, None, 0))(
            apply_fn, params, sample_points
        ).mean()
        surface_area_loss = get_implicit_surface_area_loss(
            partial(apply_fn, params), uniform_sample_points, self.alpha
        ).mean()
        loss = (
            implicit_distance_loss
            + self.eikonal_weight * eikonal_loss
            + self.surface_area_weight * surface_area_loss
        )
    ...
    def get_batch(self, train_state: ShapeTrainState, state: IGRState, key, batch_size):
        point_batch, batch_indices = samplers.sample_array(
            key, state.point_cloud, batch_size
        )
        ...
        global_samples = samplers.generate_global_samples(
            ...
            n_points=(
                self.sampling.global_samples
                if self.sampling.global_samples is not None
                else len(local_samples) // 8
            ),
        ...
        return (
            state,
            point_batch,
            jnp.concatenate([local_samples, global_samples]),
            uniform_samples,
        )

    @partial(jit, static_argnames="self")
    def step(
        self, state: ShapeTrainState, query_points, sample_points, uniform_sample_points
    ):
        (
            loss,
            (distance_loss, eikonal_loss, surface_area_loss),
        ), grads = jax.value_and_grad(self.get_loss, argnums=1, has_aux=True)(
            state.apply_fn,
            state.params,
            query_points,
            sample_points,
...
class NeuralPull:
    ...
    sampling: SamplingConfig = field(
        default_factory=lambda: SamplingConfig(samples_per_point=10)
    )
    ...
    def get_batch(
        self,
        train_state: ShapeTrainState,
        state: NeuralPullState,
        key,
        batch_size,
        *args,
    ):
        batch_indices = jrnd.choice(
            key,
            len(state.sample_points),
            (min(batch_size, len(state.sample_points)),),
            replace=False,
        )
        return (
            state,
            state.sample_points[batch_indices],
            state.target_points[batch_indices],
        )
    ...
    def batch_loss(self, apply_fn, params, sample_points, target_points):
        ...
        return neural_pull_loss + self.eikonal_weight * eikonal_loss, (
            neural_pull_loss,
            eikonal_loss,
        )
    ...
    def step(self, train_state: ShapeTrainState, sample_points, target_points):
        (loss, (distance_loss, eikonal_loss)), grads = jax.value_and_grad(

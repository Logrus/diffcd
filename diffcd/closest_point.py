...
def laplacian(f, params, query_point, z):
    ...
    x, mu = z[:-1], z[-1]
    return 0.5 * sq_norm(x - query_point) + mu * f(params, x)


@partial(jax.jit, static_argnames=("f", "newton_config"))
def closest_point_newton(
    f, params, query_point, init_point, newton_config: NewtonConfig
):
    ...
    if init_point is None:
        ...
        mu0 = (
            jnp.linalg.norm(query_point - init_point)
            / jnp.linalg.norm(g)
            * jnp.sign((query_point - init_point)[0] * g[0])
        )
    ...
    valid_kkt = check_kkt(newton_state, f(params, query_point))
...
def get_distance_derivative(f, theta, query_point, closest_point):
    ...
    A = jnp.block([[jnp.eye(len(query_point)) + mu * H, g_x[:, None]], [g_x, 0]])

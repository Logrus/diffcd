...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        ...
        for i in range(self.n_layers):
            ...
            layer_size = (
                self.layer_size
                if i + 1 not in self.skip_layers
                else self.layer_size - dim_x
            )
            x = nn.Dense(
                features=layer_size, name=f"dense_{i}", kernel_init=kernel_init
            )(x)
            x = actication_function(x)
        kernel_init_final = (
            non_zero_mean if self.geometry_init else linear.default_kernel_init
        )
        bias_init_final = jax.nn.initializers.constant(
            -self.init_radius if self.geometry_init else 0.0
        )
        x = nn.Dense(
            features=1,
            name=f"dense_{self.n_layers}",
            kernel_init=kernel_init_final,
            bias_init=bias_init_final,
        )(x)
        return x.squeeze()
